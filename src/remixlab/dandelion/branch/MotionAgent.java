 ...
 * {@link remixlab.dandelion.branch.GenericFrame} objects, but it can also handle third-party
 ...
 * {@link remixlab.bias.branch.MotionBranch}es.
 * <p>
 * The agent has two branches of the type {@code MotionBranch<MotionAction, A, ClickAction>} to handle
 * {@link remixlab.dandelion.branch.GenericFrame} object instances: {@link #eyeBranch()}, for
 * {@link remixlab.dandelion.branch.Constants.Target#EYE} (which typically has one one single
 ...
 * agent {@link remixlab.bias.branch.Profile}s are available:
 * {@link #eyeProfile()}, {@link #frameProfile()}, {@link #eyeClickProfile()}
 * and {@link #frameClickProfile()}. Refer to the {@link remixlab.bias.branch.Branch} and
 ...
 * methods take a {@link remixlab.dandelion.branch.Constants.Target} parameter type which actually
 ...
 * (DOFs) that the agent supports. See {@link remixlab.dandelion.branch.TrackballAgent},
 * {@link remixlab.dandelion.branch.WheeledMouseAgent}, {@link remixlab.dandelion.branch.JoystickAgent}
 * and {@link remixlab.dandelion.branch.HIDAgent}.
 * 
 * @see remixlab.dandelion.branch.Constants.MotionAction
 * @see remixlab.dandelion.branch.Constants.ClickAction
 */
public abstract class MotionAgent<A extends remixlab.bias.branch.Action<MotionAction>> extends GenericMotionAgent {
	 ...
	 */
	public MotionAgent(GenericScene scn, String n) {
	...
	public boolean addGrabber(Grabber frame) {
		if (frame instanceof GenericFrame)
			return addGrabber((GenericFrame) frame, ((GenericFrame) frame).isEyeFrame() ? eyeBranch : frameBranch);
	 ...
	 * {@link remixlab.dandelion.branch.TrackballAgent}, {@link remixlab.dandelion.branch.WheeledMouseAgent},
	 * {@link remixlab.dandelion.branch.JoystickAgent} and {@link remixlab.dandelion.branch.HIDAgent}.
	 */
	 ...
	 * (for {@link remixlab.dandelion.branch.Constants.Target#EYE}); or, for other interactive-frame instances
	 * (for {@link remixlab.dandelion.branch.Constants.Target#FRAME}).
	 * <p>
	 * {@code A} is the ({@code Action<MotionAction>}) used to parameterize the agent. See
	 * {@link remixlab.dandelion.agent.TrackballMouseAgent}, {@link remixlab.dandelion.branch.WheeledMouseAgent},
	 * {@link remixlab.dandelion.agent.JoystickMouseAgent} and {@link remixlab.dandelion.agent.HIDMouseAgent}.
	 * 
	 * @see #eyeProfile()
	 * @see #frameProfile()
	 */
	protected remixlab.bias.branch.Profile<MotionAction, MotionShortcut, A> motionProfile(Target target) {
	 ...
	 * @see #eyeBranch()
	 */
	protected remixlab.bias.branch.Profile<MotionAction, MotionShortcut, A> eyeProfile() {
	 ...
	 * @see #frameBranch()
	 */
	protected remixlab.bias.branch.Profile<MotionAction, MotionShortcut, A> frameProfile() {
	 ...
	 * to {@link remixlab.dandelion.branch.Constants.ClickAction} mappings (bindings) either the
	 ...
	 */
	protected remixlab.bias.branch.Profile<MotionAction, ClickShortcut, ClickAction> clickProfile(Target target) {
	 ...
	 * <p>
	 * The profile defines customizable {@link remixlab.bias.core.ClickShortcut} to
	 * {@link remixlab.dandelion.branch.Constants.ClickAction} mappings (bindings) for the
	 * {@link remixlab.dandelion.core.GrabberScene#eyeFrame()} instance.
	 * 
	 * @see #eyeBranch()
	 */
	protected remixlab.bias.branch.Profile<MotionAction, ClickShortcut, ClickAction> eyeClickProfile() {
	 ...
	 * <p>
	 * The profile defines customizable {@link remixlab.bias.core.ClickShortcut} to
	 * {@link remixlab.dandelion.branch.Constants.ClickAction} mappings (bindings) for interactive-frame instances
	 * different that the {@link remixlab.dandelion.core.GrabberScene#eyeFrame()}.
	 * 
	 * @see #frameBranch()
	 */
	protected remixlab.bias.branch.Profile<MotionAction, ClickShortcut, ClickAction> frameClickProfile() {
